...
import OverseasProductPanel from '../components/drug/OverseasProductPanel.vue'
...
<template>
  ...
  <div v-else-if="detail.error.value && !detail.drug.value" class="notice">
    {{ detail.error.value }}
  </div>
  <section v-else-if="detail.drug.value" class="detail-page">
    ...
    <ContraindicationPanel
      v-else-if="detail.activeTab.value === 'dur'"
    ...
    />
    <OverseasProductPanel
      v-else
      :loading="detail.overseasProductsLoading.value"
      :result="detail.overseasProducts.value"
      @retry="detail.loadOverseasProducts"
    />
