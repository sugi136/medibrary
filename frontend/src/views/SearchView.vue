...
<template>
  <section class="search-hero">
    <div class="hero-copy">
      ...
      <p>
        약 이름 또는 알약의 모양과 색깔로 검색하고, 효능·부작용·병용 주의 정보를 한곳에서
        살펴보세요.
      </p>
    </div>
    ...
    <div class="search-panel card">
      <div class="mode-tabs" role="tablist" aria-label="검색 방식">
        <button
          :class="{ active: searchState.mode.value === 'name' }"
          type="button"
          @click="searchState.mode.value = 'name'"
        >
          이름으로 검색
        </button>
        <button
          :class="{ active: searchState.mode.value === 'appearance' }"
          type="button"
          @click="searchState.mode.value = 'appearance'"
        >
          모양·색깔로 검색
        </button>
      </div>
      <form @submit.prevent="searchState.search">
        ...
        <div v-else class="appearance-grid">
          <select v-model="searchState.shape.value" class="select" aria-label="약 모양">
...
.search-hero {
  ...
  margin: -44px calc((1120px - 100vw) / 2) 42px;
...
.mode-tabs button.active {
  color: var(--ink);
  background: #fff;
  box-shadow: 0 3px 8px rgba(77, 139, 163, 0.1);
}
.search-row {
  display: grid;
